...
public class AdaptiveLoadBalance implements LoadBalance {
    ...
    public <T> Invoker<T> select(List<Invoker<T>> invokers, URL url, Invocation invocation) throws RpcException {
        ...
        for (SnapshotStats stats : sortStats) {
            ...
            int idle = threads - stats.getActiveCount();
            RuntimeInfo runtimeInfo = stats.getServerStats().getRuntimeInfo();
            if (runtimeInfo != null) {
                idle /= runtimeInfo.getAvailableProcessors();
            }
            if (idle > maxIdle) {
                maxIdle = idle;
                mostIdleStats = stats;
            ...
            if (waits > threads *  conf.getMaxRateOfWaitingRequests()
                // || runtimeInfo != null && runtimeInfo.getProcessCpuLoad() > conf.getMaxProcessCpuLoad()
                // (ThreadLocalRandom.current().nextInt() & 0x00000001) == 0
                    ) {
                continue;
            }
            ...
            if (OPEN_LOGGER &&
                ...
                logger.info("  SELECT " + address +
                            ...
                            (runtimeInfo != null ?
                                    ", load=" + runtimeInfo.getProcessCpuLoad() : "")
                           );
            }
            ...
            if (OPEN_LOGGER &&
                ...
                logger.info("  MOST " + address +
                            ...
                            (conf.isOpenRuntimeStats() ?
                                    ", load=" + mostIdleStats.getServerStats()
                                                             .getRuntimeInfo()
                                                             .getProcessCpuLoad() : "")
                           );
            }
